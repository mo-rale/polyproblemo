...
// Quiz questions including multiplication and division
const quizQuestions = [
  // Addition questions
...
// Parse polynomial to array of terms with coefficients and exponents
...
function formatTermForDivision(term) {
  if (term.coefficient === 0) return '0';
  
  let result = '';
  if (term.coefficient < 0) result += '-';
  if (Math.abs(term.coefficient) !== 1 || term.exponent === 0) {
    result += Math.abs(term.coefficient);
  }
...
function formatPolynomialArray(polyArray) {
  ...
  for (let i = 0; i < polyArray.length; i++) {
    ...
    if (power === 0) {
      ...
      result += sign + coeffStr + "x";
    } else {
      result += sign + coeffStr + "x^" + power;
    }
  }
...
// Helper functions for polynomial arrays
...
// Format array of terms as a polynomial string
